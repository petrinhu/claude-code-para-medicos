...
>
> - **Honestidade depende de limiar de relevância (CRÍTICO p/ o clímax):** o ChromaDB sempre devolve os N vizinhos mais próximos — "tema ausente → lista vazia" só funciona se `busca_service.buscar()` aplicar um corte de score/distância (comportamento prometido na aula_29, mas cujo limiar nunca foi cravado). Confirmar na pré-gravação que `buscar("como tratar diabetes tipo 2 com metformina")` no corpus de teste devolve lista vazia ANTES de gravar. Se não devolver, o corte não existe/está frouxo — pedir ao Claude para revisar o limiar em `busca_service`/`chroma_repositorio` antes (é pré-condição do clímax, não conteúdo de aula). Se diabetes (tema médico, semanticamente mais perto) não cair vazio de forma estável, usar um tema mais distante como fallback.
