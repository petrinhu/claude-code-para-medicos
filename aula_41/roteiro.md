...
> - **PRÉ-REQUISITO BLOQUEANTE DE GRAVAÇÃO — estado do `clinmd_tribe/` no disco (CRÍTICO, achado C1/I2/I3 dos revisores):** no repo do curso, `clinmd_tribe/` é scaffold (camadas vazias, `domain/__init__.py` e `tests/__init__.py` com 0 linhas). As Seções 5 e 6 dependem de o app-piloto da gravação ter calculadoras committadas no Domínio + testes verdes. **NÃO gravar as Seções 5-6 sem decidir um dos dois caminhos abaixo.** A escolha é do Dr. Petrus; as duas falas estão escritas para não improvisar ao vivo.
>   - **CAMINHO A (recomendado) — app-piloto real:** ter pelo menos 2-3 calculadoras modulares no `domain/` (incluindo o MELD, da aula_24) + suíte pytest VERDE committada. Aí os prompts do corpo rodam de verdade: a fala "Toquei um arquivo. O do MELD, no Domínio. Mais nada." (Seção 6) e os testes verdes do Prompt 3/4 são reais. **Alinhar a lista de calculadoras do Prompt 2, item 1 (Seção 5) EXATAMENTE com as que estiverem committadas** — citar uma calculadora ausente faz o laudo dizer "não encontrado" ao vivo (achado I3).
>   - **CAMINHO B — scaffold/hipótese:** trocar o prompt do teste (Seção 6) de "Faça a mudança... quantos arquivos você precisou tocar" por **"Na estrutura saudável que a gente planejou para o ClinMd-Tribe, se eu mudasse um parâmetro da regra do MELD, em quantos arquivos isso mexeria? Responda como hipótese, sem código."** — e a fala da Seção 6 vira: **"O Claude confirma: na casa arrumada, seria um arquivo só — o do MELD, no Domínio. Mais nada."** No Prompt 3 (arrumação), trocar "RODE os testes (pytest)" por "me explique como você CONFIRMARIA, com os testes, que nada quebrou" (descrição do hábito, não execução). O Prompt 2 (ronda) vira laudo da estrutura planejada, não auditoria do disco vazio.
>   - **Em ambos os caminhos:** gravar fala pronta nos 2 cenários da Seção 5 (laudo todo verde / laudo achou um amarelo-vermelho), igual ao padrão laudo-limpo/achou da aula_39 e cofre-vazio da aula_40.
